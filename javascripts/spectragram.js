...
(function ($, window, document, undefined) {
  const Instagram = {
    ...
    getSelfMedia: function () {
      const self = this
      const getData = '/users/self/media/recent/?&access_token=' + self.accessToken
    ...
    getRecentMedia: function () {
      const self = this
      const getData = '/users/' + userID + '/media/recent/?' + self.userCredentials
    ...
    getUserFeed: function () {
      const self = this
      const getData = '/users/search?q=' + self.options.query + '&count=' + self.options.max + '&access_token=' + self.accessToken + ''
      let isUsernameValid = false

      self.fetch(getData).done(function (results) {
        if (results.data.length) {
          // Only request media for exact match of username
          for (let length = results.data.length, i = 0; i < length; i++) {
    ...
    getPopular: function () {
      const self = this
      const getData = '/media/popular?client_id=' + self.userCredentials
    ...
    getRecentTagged: function () {
      const self = this
      const getData = '/tags/' + self.options.query + '/media/recent?client_id=' + self.userCredentials
    ...
    fetch: function (getData) {
      const getUrl = this.API_URL + getData
  ...
  jQuery.fn.spectragram = function (method, options) {
    if (jQuery.fn.spectragram.accessData.clientID) {
      this.each(function () {
        const instagram = Object.create(Instagram)
